...
class MonthAdapter extends BaseAdapter {
  ...
  public TextView getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
    ...
    if (calendarConstraints.getDateValidator().isValid(date)) {
      ...
      Map<Long, Integer> dayToItemBackgroundColors = calendarConstraints.getDayToItemBackgroundColor();
      if (dayToItemBackgroundColors != null && dayToItemBackgroundColors.containsKey(date)) {
        calendarStyle.customDay.get(dayToItemBackgroundColors.get(date)).styleItem(day);
        return day;
      }
  ...
  private void initializeStyles(Context context) {
    if (calendarStyle == null) {
      Map<String, Integer> hintTitleToColor = calendarConstraints.getHintTitleToColor();
      if (hintTitleToColor != null) {
        calendarStyle = new CalendarStyle(context, new ArrayList<>(hintTitleToColor.values()));
      } else {
        calendarStyle = new CalendarStyle(context, null);
      }
    }
  }
